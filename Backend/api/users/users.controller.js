...
const contactController = require('../contacts/contacs.controller');
...
const login = (req,res,next) => {
    ...
    jwt.verify(token, "CRMTOPSECRET", (err, dataStored) => {
        if (err) return res.status(403).send('The User/Password is not correct1');
        req.user = dataStored
        next()
    })
}
